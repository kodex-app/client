...
/**
 * Open-book glyph — the ebook reader's *book contents* action.
 *
 * Deliberately not a list of rules: "Chapters" (the series' other books) sits right beside it using
 * the list icon, and two line-based marks next to each other were impossible to tell apart. A book
 * silhouette says "inside this book" at a glance, which is the actual distinction.
 */
val BookContentsIcon: ImageVector = materialIcon(name = "Filled.MenuBook") {
    // Two pages splayed from a central spine gap.
    materialPath {
        moveTo(11f, 7.5f)
        lineTo(2f, 5.5f)
        lineTo(2f, 18f)
...
/** Material "bookmark_border" glyph (outline) — the same button when the page is not bookmarked. */
