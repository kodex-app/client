...
// NOTE (Kodex): the upstream file-loading helpers (makeBook / makeZipLoader / makeDirectoryLoader /
// format detectors) were removed. Kodex constructs the book itself via an API-backed loader and calls
// `view.open(book)` directly, so the dynamic imports of pdf.js/mobi.js/comic-book.js/fb2.js/vendor/*
// (which we don't vendor) are not pulled into the bundle. Error classes are kept for API parity.
